module fft_c2c_batch_1d
  
  use fft

  implicit none
  
  type, extends(fft_type), public :: fft_c2c_batch_1d_type
    private
...
!========================================================================================!
  subroutine delete(this)
    class(fft_c2c_batch_1d_type), intent(inout) :: this
...
!========================================================================================!
  subroutine ctor(this,s,direction)
    class(fft_c2c_batch_1d_type), intent(out) :: this
    ...
    if (size(s)==0) then
      call mpi_abort_run("size(s)=0 at fft_c2c_batch_1d_type%ctor")
    end if
    
    do i=1,size(s)
      if (s(i)<2) then
        call mpi_abort_run("s(i)<2 at fft_c2c_batch_1d_type%ctor")
      end if      
    end do
    
    if ((direction.ne.fftw_forward).and.&
        (direction.ne.fftw_backward)) then
      call mpi_abort_run("invalid direction at fft_c2c_batch_1d_type%ctor")
    end if
...
!========================================================================================!
  subroutine build_plan(this)
    ...
    do i=1,n
      this%plan(i)=fftw_plan_dft_1d(this%s(i),&
                                    this%in(1,i),&
                                    this%out(1,i),&
                                    this%direction,&
                                    fftw_estimate)
...
!========================================================================================!
  subroutine execute_plan(this)
    ...
    do i=1,size(this%s)
      call dfftw_execute_dft(this%plan(i),this%in(1,i),this%out(1,i))
    end do
  ...
  subroutine set_input(this,x)
    ...
    if (size(x)==0) then
      call mpi_abort_run("size(x)=0 "//&
                         "at fft_c2c_batch_1d_type%set_input")
...
!========================================================================================!
  subroutine get_input(this,x)
    class(fft_c2c_batch_1d_type), intent(in) :: this
    ...
    if (.not.allocated(this%in)) then
      call mpi_abort_run("input array not allocated "//&
                         "at fft_c2c_batch_1d_type%get_input")
    end if
  ...
  subroutine get_output(this,x)
    ...
    if (.not.allocated(this%out)) then
      call mpi_abort_run("output array not allocated "//&
                         "at fft_c2c_batch_1d_type%get_output")
    end if
