...
We suppose that a certain sequential system is characterised by a *state* that cannot be observed. Rather, we are handed an observable process whose outcome depends on the state. Hidden Markov Models provide a tool to reconstruct the (hidden) state from observables. These models find application to many scientific fields such as thermodynamics, physics, finance and signal processing in general. 
...
The numerical results were generated using the in-house code provided at [HHM simulator](https://github.com/cifanip/quantitative-finance/tree/main/cases/04_hidden_markov_models/code).
...
This problem is often encountered in practise. All one is given is a sequence of observations from real-world data. It is hoped that the problem fits well within the HMM framework such that, after having learned $\lambda = (A,B,\pi)$ from data, one can reconstruct the underlying hidden state sequence. Through an algorithm that re-estimates the model iteratively, maximization of $P(\theta ; \lambda)$ is achieved. The interested reader is referred to [^1] for implementation details. 

## 2. Computations

To illustrate the machinery of the HMM training algorithm, also known as the Baum–Welch algorithm, I present a concrete and widely employed example. Consider the following scenario. There are two possibile hidden states ($N=2$). The observation vector is made of fuor labels ($M=4$) interpreted as follows:
...
The algorithm well be fed an observation sequence generated by given probability matrices. These are:
...
and a given initial probability vector $\pi$. The interpretation is as follows. Diagonal entries of the matrix $A$ are significantly higher than off-diagonal elements. That is, a state is more likely to remain in its current regime rather than jumping to a different state. In the matrix $B$, higher probabilities are assigned to positive returns when the underlying state is in regime $1$ and to negative returns when the underlying state is in regime $2$. From this construction we understand state $1$ and $2$ as bull and bear market regimes, respectively. Of course, in reality one does not know a priori what the state should represent. Rather, their interpretation is deduced from post-processing of the model output. 

Coming back to our numerical experiment, we generate a sequence of observation of length $T=10000$ and optimise the model on this dataset. At convergence, the trained model is
...
<p align="center"><b>Figure 2:</b> Observation values (blue dots) and corresponding optimal state sequence (black dots).</p>

From Figure 2 one interprets the state values as bull and bear market regimes. When the state value is high the returns are more likely to be positive, and the opposite is true for low state values. The extent to which extreme values deviate from this patter is encoded in the probability matrix $B$. 
...
This provides a graphical illustration of the transition matrix $A$. Clearly, the state is clustered in long sequences of consecutive high or low state values. Moreover, lower state values are more likely than higher ones since $A(2,2)>A(1,1)$. 

## 3. Limitations

While HHM is a powerful tool to reconstruct hidden structures, as all models it comes with limitations. Here is a list of some of the shortcomings of HHM.
...
2. Observations may depend on past values even when conditioned on the current state.
3. The dimension of the hidden state $N$ is not known a priori. Tuning of this parameter is needed in order to avoid either overfitting or underfitting.
