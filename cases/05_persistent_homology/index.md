...
Data generated by a system or a process does not come in the form of simplicial complexes. Therefore, the first step towords computation is to construct such complexes. This is in itslef is a rather large topic, which we will not touch upon here. We will just mention that such constructions take as input a sample $S$, subset of some metric space $X$, and a scale $r$. A widely used construction is the so-called **Rips complex**. We write Rips$(S,r)$ to indicate of such complex at scale $r$. 
...
Thus, the computation of $\mathcal{b}_p$ boils down to linear algebra, provided $n_p$ and the baoundary maps. 
