...
The Betti numbers can be computed as follows. First, by the rank-nullity theorem applied to the linear map $\partial_p : C_p(K,\mathbb{F}) \to C_{p-1}(K,\mathbb{F})$ it follows that dim $C_p(K,\mathbb{F}) = dim \text{ker} \partial_p + dim rank \partial_p$
